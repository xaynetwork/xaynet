...
# sort import
isort --check-only --indent=4 -rc setup.py conftest.py benchmarks xain && echo "===> isort says: well done <===" &&

# format code
black --check --exclude "xain/grpc/.*_pb2.*" setup.py conftest.py benchmarks xain && echo "===> black says: well done <===" &&

# check format of proto files
clang_format && echo "===> clang-format says: well done <===" &&

# lint
pylint --rcfile=pylint.ini benchmarks xain && echo "===> pylint says: well done <===" &&

# type checks
mypy --ignore-missing-imports benchmarks xain && echo "===> mypy says: well done <===" &&
