...
pub fn parse_4bytes_model(c: &mut Criterion) {
    let vector_4bytes = utils::serialize_or_parse_model::make_vector_4bytes();
...
pub fn parse_100kB_model(c: &mut Criterion) {
    let vector_100kB = utils::serialize_or_parse_model::make_vector_100kB();
    c.bench_function("parse model (bounded) from 100kB vector", |bench| {
        bench.iter(|| Model::from_primitives_bounded(black_box(&mut vector_100kB.clone().into_iter())))
    });
}

pub fn parse_1MB_model(c: &mut Criterion) {
    let vector_1MB = utils::serialize_or_parse_model::make_vector_1MB();
