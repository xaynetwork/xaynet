    ...
    fn poll_aggregation(&mut self, cx: &mut Context) {
        ...
        let result = match Pin::new(&mut future).poll(cx) {
            ...
            Poll::Ready(Err(e)) => {
                ...
                Err(ServiceHandleError::Request(e))
            }
            Poll::Pending => {
    ...
    A: Aggregator,
{
    ...
    fn send_request<P>(payload: P, tx: &UnboundedSender<P>) -> Result<(), TransmitError> {
        trace!("send request to the service");
        if tx.send(payload).is_err() {
            warn!("failed to send request: channel closed");
            Err(TransmitError::Request)
        } else {
            trace!("request sent");
            Ok(())
        }
    ...
    async fn recv_response<R>(rx: oneshot::Receiver<R>) -> Result<R, TransmitError> {
        rx.await.map_err(|_| {
...
//     Selection(#[from] A::Error),
// }

#[derive(Error, Debug)]
pub enum ServiceHandleError<E>
where
    E: Error,
{
    #[error("failed to send the request or receive the response")]
