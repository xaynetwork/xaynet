...
type SumParticipantPublicKey = box_::PublicKey;
...
/// the update participants. It maps the encrypted masking seeds of
/// all the update participants to each sum participant.
type UpdateDict =
...
pub struct Coordinator {
    ...
    dict_update: UpdateDict,
    /// Dictionary built during the sum2 phase.
    dict_mask: Counter<MaskHash>,
}
...
impl Default for Coordinator {
    fn default() -> Self {
        ...
        let dict_update = UpdateDict::new();
...
impl Coordinator {
    ...
    fn handle_message_update(&mut self, message: &[u8]) -> Result<(), PetError> {
        ...
        self.add_seed_dict(msg.encr_pk(), msg.dict_seed())?;
        Ok(())
    }
    ...
    fn freeze_dict_sum(&mut self) {
        self.dict_update = self
            .dict_sum
            .keys()
            .map(|pk| (*pk, SeedDict::new()))
            .collect();
    }
    ...
    fn add_seed_dict(
        ...
        .then(|| {
            dict_seed.iter().for_each(|(pk_sum, seed)| {
                self.dict_update
                    .get_mut(pk_sum)
                    .unwrap()
    ...
    fn clear_round_dicts(&mut self) {
        ...
        self.dict_update.clear();
        self.dict_update.shrink_to_fit();
...
mod tests {
    ...
    fn test_coordinator() {
        ...
        assert_eq!(coord.dict_update, UpdateDict::new());
    ...
    fn test_dict_sum() {
        ...
        assert!(coord.dict_update.is_empty());
        assert!(coord.has_enough_sums());
    ...
    fn generate_update(dict_sum: &SumDict) -> (UpdateParticipantPublicKey, SeedDict) {
        let seed = randombytes(32);
    ...
    fn auxiliary_update(
        ...
        SumDict,
        Vec<(UpdateParticipantPublicKey, SeedDict)>,
        UpdateDict,
    ) {
        ...
        let updates = iter::repeat_with(|| generate_update(&dict_sum))
            .take(min_update)
            .collect::<Vec<(UpdateParticipantPublicKey, SeedDict)>>();

        let mut dict_update = UpdateDict::new();
        for sum_encr_pk in dict_sum.keys() {
            ...
            dict_update.insert(*sum_encr_pk, sum_participant_seeds);
        }
        (dict_sum, updates, dict_update)
    }
    ...
    fn test_dict_update() {
        ...
        let (dict_sum, updates, dict_update) = auxiliary_update(coord.min_sum, coord.min_update);
        ...
        for (encr_pk, dict_update) in updates.iter() {
            assert!(!coord.has_enough_updates());
            coord.add_seed_dict(encr_pk, dict_update).unwrap();
        }
        assert_eq!(coord.dict_update, dict_update);
        assert!(coord.has_enough_updates());
    ...
    fn test_transitions() {
        ...
        let (dict_sum, _, dict_update) = auxiliary_update(coord.min_sum, coord.min_update);
