...
use crate::coordserde::serde::{serde_counter, serde_sodiumoxide::*};
...
/// participant.
pub(crate) type SumParticipantEphemeralPublicKey = box_::PublicKey;

/// A public key that identifies a sum participant.
pub(crate) type SumParticipantPublicKey = box_::PublicKey;
...
/// key that participant generated.
pub(crate) type SumDict = HashMap<SumParticipantPublicKey, SumParticipantEphemeralPublicKey>;

/// Public key that identifies an update participant
pub(crate) type UpdateParticipantPublicKey = box_::PublicKey;

/// Local seed dictionaries are sent by update participants. They
/// contain the participant's seed, encrypted with the ephemeral
/// public key of each sum participant.
pub(crate) type LocalSeedDict = HashMap<SumParticipantPublicKey, EncryptedMaskingSeed>;

/// The global seed dictionary is built from the local seed
/// dictionaries sent by the update participants. It maps the
/// encrypted masking seeds of all the update participants to each sum
/// participant.
pub(crate) type SeedDict =
...
pub struct Coordinator {
    // credentials
    #[serde(
        serialize_with = "u8vec_as_hex",
        deserialize_with = "enc_pubkey_from_hex"
    )]
    pub(crate) encr_pk: box_::PublicKey, // 32 bytes
    #[serde(
        serialize_with = "u8vec_as_hex",
        deserialize_with = "enc_seckey_from_hex"
    )]
    pub(crate) encr_sk: box_::SecretKey, // 32 bytes
    #[serde(
        serialize_with = "u8vec_as_hex",
        deserialize_with = "sign_pubkey_from_hex"
    )]
    pub(crate) sign_pk: sign::PublicKey, // 32 bytes
    #[serde(
        serialize_with = "u8vec_as_hex",
        deserialize_with = "sign_seckey_from_hex"
    )]
    pub(crate) sign_sk: sign::SecretKey, // 64 bytes
    ...
    #[serde(serialize_with = "se_sum_dict", deserialize_with = "de_sum_dict")]
    pub(crate) dict_sum: SumDict,
    /// Dictionary built during the update phase.
    #[serde(serialize_with = "se_seed_dict", deserialize_with = "de_seed_dict")]
    pub(crate) dict_seed: SeedDict,
    /// Dictionary built during the sum2 phase.
    #[serde(with = "serde_counter")]
    pub(crate) dict_mask: Counter<MaskHash>,
}
...
impl Default for Coordinator {
    fn default() -> Self {
        ...
        let dict_sum = SumDict::new();
        let dict_seed = SeedDict::new();
...
impl Coordinator {
    ...
    fn handle_message_sum(&mut self, message: &[u8]) -> Result<(), PetError> {
        ...
        self.add_sum_participant(msg.encr_pk(), msg.ephm_pk());
        Ok(())
    }
    ...
    fn handle_message_update(&mut self, message: &[u8]) -> Result<(), PetError> {
        ...
        self.add_local_seed_dict(msg.encr_pk(), msg.local_dict_seed())?;
        Ok(())
    }
    ...
    fn handle_message_sum2(&mut self, message: &[u8]) -> Result<(), PetError> {
        ...
        Self::validate_certificate(msg.certificate())?;
        self.validate_task_sum(msg.signature_sum(), msg.sign_pk())?;
        self.add_mask_hash(msg.mask_hash());
        Ok(())
    }
    ...
    fn add_sum_participant(&mut self, encr_pk: &box_::PublicKey, ephm_pk: &box_::PublicKey) {
    ...
    /// Add the given seed dictionary to the update dictionary
    fn add_local_seed_dict(
        &mut self,
        pk_update: &box_::PublicKey,
        local_dict_seed: &LocalSeedDict,
    ) -> Result<(), PetError> {
        (local_dict_seed
            .keys()
            .collect::<HashSet<&box_::PublicKey>>()
            == self.dict_sum.keys().collect::<HashSet<&box_::PublicKey>>())
        .then(|| {
            local_dict_seed.iter().for_each(|(pk_sum, seed)| {
                self.dict_seed
                    .get_mut(pk_sum)
                    .unwrap()
    ...
    fn freeze_dict_mask(&self) -> Result<MaskHash, PetError> {
...
mod tests {
    ...
    fn test_coordinator() {
        ...
        assert_eq!(coord.dict_sum, SumDict::new());
        assert_eq!(coord.dict_seed, SeedDict::new());
    ...
    fn generate_update(dict_sum: &SumDict) -> (UpdateParticipantPublicKey, LocalSeedDict) {
        let seed = randombytes(32);
        let pk = box_::PublicKey::from_slice(&randombytes(32)).unwrap();
        let seed_dict = dict_sum
            .iter()
            .map(|(sum_encr_pk, sum_ephm_pk)| (*sum_encr_pk, sealedbox::seal(&seed, sum_ephm_pk)))
    ...
    fn auxiliary_update(
        ...
        let updates = iter::repeat_with(|| generate_update(&dict_sum))
            .take(min_update)
            .collect::<Vec<(UpdateParticipantPublicKey, LocalSeedDict)>>();

        let mut dict_seed = SeedDict::new();
        for sum_encr_pk in dict_sum.keys() {
            // Dictionary of all the encrypted seeds for that participant
            let sum_participant_seeds = updates
                .iter()
                .map(|(upd_encr_pk, local_dict_seed)| {
                    (
                        *upd_encr_pk,
                        local_dict_seed.get(sum_encr_pk).unwrap().clone(),
                    )
                })
                .collect::<HashMap<UpdateParticipantPublicKey, EncryptedMaskingSeed>>();
            dict_seed.insert(*sum_encr_pk, sum_participant_seeds);
        }
    ...
    fn test_dict_seed() {
        ...
        for (encr_pk, dict_seed) in updates.iter() {
            assert!(!coord.has_enough_seeds());
            coord.add_local_seed_dict(encr_pk, dict_seed).unwrap();
        }
