...
def identitiy_transformer(x: np.ndarray, y: np.ndarray):
    return x, y


@pytest.fixture
def mock_random_splits_2_dataset() -> FederatedDataset:
    """Federated dataset mock with two partitions with no transformation"""
    return data.create_federated_dataset(
        keras_dataset=MockKerasDataset(),
        num_partitions=2,
        validation_set_size=60,
        transformers=[identitiy_transformer],
    )
